...
export class ClientPackageComponent implements OnInit {
  ...
  addRow(): void {
    const tagControl = this.frm.get('frmOrdDetails')?.value;
    let userName = this.frm.controls['frmOrdDetails'].value.prdProductId;
    console.log(userName);
  
    this.rows.push(tagControl);
  }
  removeRow(index: number): void {
    this.rows.splice(index, 1); // 'index' is the index of the row you want to remove
  }
